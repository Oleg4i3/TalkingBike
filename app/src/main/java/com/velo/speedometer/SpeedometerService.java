...
public class SpeedometerService extends Service {
    ...
    // Intent actions (from notification buttons or Activity)
    ...
    // ── Settings (reloaded on each start) ────────────────────────────────────
    ...
    public void startTracking() {
        ...
        Notification notification = buildNotification(0, 0, 0);
    ...
    /** Called from notification "📢" button or volume double-press. */
    ...
    private void speak(String text) {
        ...
        // STREAM_ALARM cuts through wind noise
    ...
    private Notification buildNotification(float speed, float avg, float dist) {
        ...
        // Announce action
        Intent announceIntent = new Intent(this, SpeedometerService.class)
                .setAction(ACTION_ANNOUNCE);
        PendingIntent announcePi = PendingIntent.getService(this, 1, announceIntent, flags);

        // Stop action
        Intent stopIntent = new Intent(this, SpeedometerService.class)
                .setAction(ACTION_STOP);
        PendingIntent stopPi = PendingIntent.getService(this, 2, stopIntent, flags);

        String content = tracking
                ? String.format(Locale.US, "%.0f km/h  |  avg %.0f  |  %.2f km", speed, avg, dist)
                : "Tap to open";

        return new NotificationCompat.Builder(this, CHANNEL_ID)
                .setContentTitle("🚴 VeloSpeedometer")
    ...
    private void updateNotification(float speed, float avg, float dist) {
        Notification n = buildNotification(speed, avg, dist);
    ...
    private void loadSettings() {
        ...
        float alpha       = p.getFloat("ema_alpha", 0.3f);
